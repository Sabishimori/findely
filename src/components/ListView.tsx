            ...
            return (
              <div
                ...
                className={`rounded-[28px] p-5.5 border shadow-xs hover:shadow-xl hover:-translate-y-0.5 transition-all cursor-pointer flex flex-col justify-between group ${
                  isDarkMode
                    ? "bg-[#152216] border-white/10 hover:border-[#A9C632]"
              ...
              >
                <div className="space-y-3.5">
                  ...
                  <div className="flex items-start justify-between gap-2.5">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-12 h-12 rounded-2xl bg-[#F7F9F2] dark:bg-white/10 border border-[#C8D2A6] dark:border-white/10 flex items-center justify-center p-2 overflow-hidden flex-shrink-0 shadow-2xs">
                        <img
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-[#546E50] dark:text-[#D2E0CC] flex items-center gap-1 truncate mt-0.5">
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-full hover:bg-red-500/10 text-[#546E50] hover:text-red-500 dark:text-[#D2E0CC] dark:hover:text-red-400 transition-colors"
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-2xs ${
                          hasJobs
                            ...
                            : "bg-gray-100 dark:bg-white/10 text-gray-400 dark:text-[#9EB296]"
                        }`}
                      >
                  ...
                  {/* Company Description */}
                  {company.description && (
                    <p className="text-xs text-[#2D442A] dark:text-[#D2E0CC] line-clamp-2 leading-relaxed">
                      {company.description}
                    </p>
                  )}
                  ...
                  {company.jobTitles && company.jobTitles.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 pt-0.5">
                      {company.jobTitles.slice(0, 3).map((title, i) => (
                        <span
                          key={i}
                          className={`px-2.5 py-1 rounded-xl text-[11px] font-medium truncate max-w-[210px] border ${
                            isDarkMode 
                              ? "bg-white/10 text-white border-white/10" 
                              : "bg-[#F7F9F2] text-[#1D2E1B] border-[#C8D2A6]"
                          }`}
                        >
                          {title}
                        </span>
                ...
                {/* Footer Action Strip */}
                <div className="mt-4 pt-3 border-t border-[#C8D2A6] dark:border-white/10 flex items-center justify-between text-xs text-[#1D2E1B] dark:text-[#D2E0CC] group-hover:text-[#A9C632] font-semibold transition-colors">
